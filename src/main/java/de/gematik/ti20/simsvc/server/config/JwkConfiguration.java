...
public class JwkConfiguration {
   ...
   */
  @Bean(name = "publicJwk")
  public JsonWebKeySet jwkSource(final KeyStore poppKeyStore) {
    try {
      final String firstAlias = poppKeyStore.aliases().nextElement();
      log.info("Using alias {} as publicJwk", firstAlias);

      final PublicKey publicKey = poppKeyStore.getCertificate(firstAlias).getPublicKey();
